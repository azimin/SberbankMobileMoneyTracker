...
- (NSArray*)analogousColorsWithStepAngle:(CGFloat)stepAngle pairCount:(int)pairs {
	...
	for (int i = 1; i <= pairs; ++i) {
		CGFloat a1 = fmodf(stepAngle * i, 360.0f);
		
		CGFloat h1 = fmodf(h + a1, 360.0f);
		CGFloat h2 = fmodf(h + 360.0f - a1, 360.0f);
		
		[colors addObject:[UIColor colorWithHue:h1 saturation:s brightness:v alpha:a1]];
		[colors addObject:[UIColor colorWithHue:h2 saturation:s brightness:v alpha:a1]];
	}
